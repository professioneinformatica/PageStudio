...
public abstract class PageElement : IPageElement
{
    ...
    private readonly SKRect[] handleRects = new SKRect[8]; // 8 handle: 4 angoli, 4 lati
    ...
    private float ZoomLevel => Page.Document.CanvasInteractor.ZoomManager.Level;
    ...
    {
        for (var i = 0; i < handleRects.Length; i++)
        {
            var rect = handleRects[i];
            var hitRect = new SKRect(rect.Left, rect.Top, rect.Right, rect.Bottom);
    ...
    public virtual bool RemoveChild(Guid elementId)
    {
        ...
        if (element != null) return RemoveChild(element);
    ...
    public virtual IPageElement? GetChildren(Guid elementId) => _children.FirstOrDefault(e => e.Id == elementId);
    ...
    {
        var maxSize = 12f / ZoomLevel;
        var minSize = 4f / ZoomLevel;
        var size = 8f;
    ...
    protected virtual void RenderCore(IGraphicsContext graphics)
    {
        ...
        if (IsSelected)
            ...
            if (graphics is GraphicsContext skiaContext)
            {
                ...
                using var selectionPaint = new SKPaint
                {
                    ...
                    PathEffect = SKPathEffect.CreateDash([3 / ZoomLevel, 3 / ZoomLevel], 0)
                };

                var w = (float)Width.Value;
                var h = (float)Height.Value;
                // plot the selection dashed border
                ...
                var handleBorder = new SKPaint { Color = SKColors.DeepSkyBlue, Style = SKPaintStyle.Stroke, StrokeWidth = 2 / ZoomLevel };
                for (var i = 0; i < 8; i++)
                {
    ...
    /// </summary>
    protected void UpdateModifiedTime() => ModifiedAt = DateTime.UtcNow;
    ...
    public string Name
    {
        ...
        set
        {
            ...
            // Rimosso il vincolo di univocità globale.
            // Opzionalmente potremmo verificare l'univocità tra fratelli,
            // ma per ora permettiamo tutto come richiesto.
